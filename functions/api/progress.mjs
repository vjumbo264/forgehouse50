...
// (user_reading_days), and each row flags whether it is the quiz gate.
...
export async function onRequestGet({ request, env }) {
  ...
  const [progressRows, noteCounts, stats, quizRows] = await Promise.all([
    ...
    env.DB.prepare('SELECT day_number, MAX(passed) AS passed, COUNT(*) AS attempts FROM quiz_attempts WHERE user_id = ? GROUP BY day_number').bind(user.id).all(),
  ]);
  ...
  const daysOut = (calRows || []).map(d => {
    ...
    const q = quizByDay.get(d.day_number);
